...
- **v1.3 (2026-05-25)** — **제목 카드 + 음악 자동 ramp-up**. ③-2 체크박스
  + 텍스트 + 초(1-8)·체크 시 영상 맨 앞에 검은 배경 + 중앙 큰 글자(120px)·
  자체 0.5초 fade in/out·제목 동안 음악 작게(0.2)·제목 끝나기 0.5초 전부터
  1.5초간 천천히 원래 음량(0.85)으로 ramp-up. BGM(자동)·본인 음악 파일
  모두 동일 적용. 메타 JSON `title.text`·`duration_sec`·`audio_duck_ramp_sec`.
  CDN 변경 0·INVARIANT 보존.
- **v1.2 (2026-05-25)** — **자막 burn-in** 시스템 도입. ④-2 textarea에 한 줄당
  한 컷 자막 입력·자동 정렬된 미디어 순서대로 매칭. 화면 하단 ~13% 위치·
  반투명 검은 배경 + 흰색 굵은 글자 + 검은 외곽선·자동 줄바꿈(최대 4줄)·
  한국어 폰트(system). cut hold 중에만 표시(트랜지션 시 안 보임). 메타 JSON
  `media_order[].caption` + `captions_used`/`captions_total_slots` 기록.
  CDN 변경 0·INVARIANT 보존.
- **v1.1 (2026-05-25)** — **동영상은 자기 길이 전체가 사용됨** (v1.0의 컷당
  초 clamp 번복). 사진은 ⑤번 "사진 한 장당 초" 그대로·동영상만 전체 길이.
  안전 클램프 1개 영상 = [0.6초, 10분]. 메타 JSON `media_order`에
  `hold_sec` (실제 사용된 길이) 추가. CDN 변경 0·INVARIANT 보존.
